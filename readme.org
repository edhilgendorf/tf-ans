...
#+end_src
**** Attach a Policy [[./terraform/ecr.tf]]
:PROPERTIES:
:header-args: :tangle terraform/ecr.tf
:END:
#+begin_src json
...
#+end_src
**** Attach an Image :console:
- This is done via UI, subject to change once Jenkins is setup.
- https://console.aws.amazon.com/ecr/repositories?region=us-east-1
- Select ~docker_images~ repo
- Select ~View push commands~
  ...
  4. push to AWS repo
*** ECS :ecs:
- ECS is comprised of: Cluster, Task and services
**** [[./terraform/policies/terraform_user.json]]
:PROPERTIES:
:header-args: :tangle terraform/policies/terraform_user.json
:END:
#+begin_src json
        "elasticfilesystem:*",
#+end_src
**** Cluster [[./terraform/ecs.tf]]
:PROPERTIES:
:header-args: :tangle terraform/ecs.tf
:END:
#+begin_src json
...
*** [[./terraform/outputs.tf]] :untangled:outputs:
:PROPERTIES:
...
- Got an error where I could not delete my resources due to: ~Route tables are main and cannot be deleted~ in the Master-Region-RT
- Had to manually delete the corresponding VPC and all dependencies from within AWS Console
