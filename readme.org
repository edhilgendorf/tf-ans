...
#+end_src
**  /*Will be instead attaching EFS*/ Create S3 Bucket for storage :s3:
- Attaching an S3 bucket to hold public key pairs for SSH access and other miscellaneous data that may be needed during/post provisioning.
...
:header-args: :tangle terraform/scripts/cloud-init.yaml
:END:
- We need to get public SSH keys synced from master. Current plan is to add S3 ([[*Create S3 Bucket for storage][Create S3 Bucket for storage]]) IAM ([[*Create IAM Role for EC2 Access][Create IAM Role for EC2 Access]]) roles to the EC2 instances so they can add and download keys via AWS CLI ([[*Use AWS CLI][Use AWS CLI]])
...
keyed_groups:
  - key: tags
    prefix: tag
#+end_src
* IAM Base Template end :policy_end:
[[./terraform/policies/base.json]]
#+begin_src json
      ],
      "Effect": "Allow",
